...
BEGIN
    ...
    BEGIN
		...
		BEGIN
			...
			SELECT CONCAT('Sucesso na inserção da bola: ',PARAM_NUMERO) AS MENSAGEM;
		END;
		ELSE
			SELECT CONCAT('O VALOR INSERIDO: ',PARAM_QUANTIDADE, ' DEVE SER MAIOR QUE 0') AS MENSAGEM;
		END IF;
    END;
    else
...
BEGIN
	...
	BEGIN
		SELECT IFNULL(SUM(QUANTIDADE),0) INTO QUANTIDADE_ATUAL 
		FROM SACOLA WHERE NUMERO = PARAM_NUMERO;
		...
		BEGIN
            ...
            BEGIN 
                ...
                SELECT CONCAT('SUCESSO NA ALTERAÇÃO DA QUANTIDADE DE BOLAS DE NUMERO: ', PARAM_NUMERO) AS MENSAGEM;
			END;
            ELSE
...
DROP PROCEDURE ALTERA_QUANTIDADE_BOLAS;